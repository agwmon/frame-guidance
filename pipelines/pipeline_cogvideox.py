...
class CogVideoXPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def __call__(
        ...
        @torch.no_grad()
        def save_vis(latents, save_path):
        ...
        if loss_fn == "style":  
            ...
            if "content_weight" in additional_inputs:
                ...
                for content_image in video:
                    _, content_embedding_ref, _ = self.csd(self.preprocess(content_image).unsqueeze(0).to(self._execution_device))
                    additional_inputs["content_embed"] = content_embedding_ref
                    content_embedding_ref = content_embedding_ref.to(self._execution_device)
                    content_embeddings.append(content_embedding_ref)
            ...
            video = [to_tensor(frame).to(self._execution_device, dtype=self.transformer.dtype) for frame in video]
            with torch.no_grad():
                video = [self.aug(frame, mode=loss_fn) for frame in video]      
